  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (isEdit && id) {
      ...
      const { data: newPg, error: insertError } = await supabase
        ...
        .select()
        .single()
      if (insertError || !newPg) { setError('Could not create PG.'); setSaving(false); return }
      pgId = newPg.id
    }
    ...
    <div>
      ...
      <div className="container-page py-8 max-w-2xl">
        ...
        <p className="text-sm text-ink-500 mb-6">
          {isEdit ? 'Update your PG details.' : 'New listings need admin approval before they appear to students.'}
        </p>
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className="input" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g. Brodipet, Guntur" />
            </div>
          </div>
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
          ...
          <div>
            ...
            <input className="input" value={contact} onChange={(e) => setContact(e.target.value)} placeholder="10-digit mobile number" />
          </div>
          <div>
          ...
          {isEdit && (
            <div>
              ...
              <div className="flex flex-wrap gap-3 mb-3">
                {images.map((img) => (
                  <img key={img.id} src={img.url} className="w-20 h-20 object-cover rounded-card border border-ink-100" />
                ))}
              </div>
              <input
                type="file"
                accept="image/*"
                disabled={uploading}
                onChange={(e) => e.target.files?.[0] && id && handleImageUpload(e.target.files[0], id)}
                className="text-sm"
              />
          ...
          {!isEdit && (
            <p className="text-xs text-ink-400">Save the PG first, then you'll be able to upload images from the edit page.</p>
          )}
