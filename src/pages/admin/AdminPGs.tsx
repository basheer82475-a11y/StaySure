...
export default function AdminPGs() {
  ...
  async function deletePg(pg: PgRow) {
    if (!window.confirm(`Delete ${pg.name}? This permanently removes its floors, rooms, beds, and booking requests.`)) return
    setActingId(pg.id)
    const { error } = await supabase.from('pgs').delete().eq('id', pg.id)
    if (error) alert(error.message || 'Could not delete this PG. Please try again.')
    setActingId(null)
    load()
  }
  ...
  return <div><Navbar /><main className="container-page py-8"><h1 className="text-2xl font-semibold text-ink-900 mb-1">PG management</h1><p className="text-sm text-ink-500 mb-6">Review partner submissions and manage published PGs.</p>
    ...
    {(tab === 'requests' || tab === 'listings') && !loading && <div className="space-y-3">{pgs.map((pg) => <article key={pg.id} className="card p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div className="flex gap-4 min-w-0">{pg.pg_images?.[0] ? <img src={pg.pg_images[0].url} alt={pg.name} className="w-20 h-20 rounded-card object-cover border border-ink-100 shrink-0" /> : <div className="w-20 h-20 rounded-card bg-ink-50 text-ink-400 text-xs flex items-center justify-center text-center shrink-0">No photo</div>}<div className="min-w-0"><p className="font-medium text-ink-900">{pg.name}</p><p className="text-sm text-ink-500 mt-1">{pg.full_address || pg.location}{pg.pincode ? ` - ${pg.pincode}` : ''}</p><p className="text-xs text-ink-500 mt-1">Owner: {pg.owner_name ?? pg.partner?.profile?.full_name ?? pg.partner?.business_name ?? 'Unknown'}{pg.partner?.profile?.phone ? ` · ${pg.partner.profile.phone}` : ''}</p><p className="text-xs text-ink-400 truncate">{pg.partner?.profile?.email}</p></div></div><div className="flex items-center gap-2 shrink-0"><StatusBadge status={pg.status} />{tab === 'requests' && <><button onClick={() => setPgStatus(pg.id, 'approved')} disabled={actingId === pg.id} className="btn-primary btn-sm">Approve</button><button onClick={() => setPgStatus(pg.id, 'rejected')} disabled={actingId === pg.id} className="btn-danger btn-sm">Reject</button></>}{tab === 'listings' && <button onClick={() => deletePg(pg)} disabled={actingId === pg.id} className="btn-danger btn-sm">{actingId === pg.id ? 'Deleting...' : 'Delete'}</button>}</div></article>)}{pgs.length === 0 && <p className="text-sm text-ink-400">{tab === 'requests' ? 'No PG requests waiting for review.' : 'No published PG listings yet.'}</p>}</div>}
